...
from model import layer as layer
...
class YOLO_V3(object):
    ...
    def build_nework(self, input_data):
        ...
        with tf.variable_scope('darknet'):
            conv = layer.convolutional(name='conv0', input_data=input_data, filters_shape=(3, 3, 3, 32),
                                          training=self.__training)
            conv = layer.convolutional(name='conv1', input_data=conv, filters_shape=(3, 3, 32, 64),
                                          training=self.__training, downsample=True)

            # residual block part0
            conv = layer.residual_block(name='residual0', input_data=conv, input_channel=64,
                                           filter_num1=32, filter_num2=64, training=self.__training)
            conv = layer.convolutional(name='conv4', input_data=conv, filters_shape=(3, 3, 64, 128),
                                          training=self.__training, downsample=True)

            # residual block part1
            conv = layer.residual_block(name='residual1', input_data=conv, input_channel=128,
                                           filter_num1=64, filter_num2=128, training=self.__training)
            conv = layer.residual_block(name='residual2', input_data=conv, input_channel=128,
                                           filter_num1=64, filter_num2=128, training=self.__training)
            conv = layer.convolutional(name='conv9', input_data=conv, filters_shape=(3, 3, 128, 256),
                                          training=self.__training, downsample=True)

            # residual block part2
            conv = layer.residual_block(name='residual3', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256, training=self.__training)
            conv = layer.residual_block(name='residual4', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256,
                                           training=self.__training)
            conv = layer.residual_block(name='residual5', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256,
                                           training=self.__training)
            conv = layer.residual_block(name='residual6', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256,
                                           training=self.__training)
            conv = layer.residual_block(name='residual7', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256,
                                           training=self.__training)
            conv = layer.residual_block(name='residual8', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256,
                                           training=self.__training)
            conv = layer.residual_block(name='residual9', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256, training=self.__training)
            darknet_route1 = layer.residual_block(name='residual10', input_data=conv, input_channel=256,
                                           filter_num1=128, filter_num2=256, training=self.__training)
            conv = layer.convolutional(name='conv26', input_data=darknet_route1, filters_shape=(3, 3, 256, 512),
                                          training=self.__training, downsample=True)

            # redidual block part3
            conv = layer.residual_block(name='residual11', input_data=conv, input_channel=512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            conv = layer.residual_block(name='residual12', input_data=conv, input_channel=512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            conv = layer.residual_block(name='residual13', input_data=conv, input_channel=512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            conv = layer.residual_block(name='residual14', input_data=conv, input_channel=512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            conv = layer.residual_block(name='residual15', input_data=conv, input_channel=512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            conv = layer.residual_block(name='residual16', input_data=conv, input_channel=512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            conv = layer.residual_block(name='residual17', input_data=conv, input_channel=512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            darknet_route2 = layer.residual_block(name='residual18', input_data=conv, input_channel= 512,
                                           filter_num1=256, filter_num2=512, training=self.__training)
            conv = layer.convolutional(name='conv43', input_data=darknet_route2, filters_shape=(3, 3, 512, 1024),
                                          training=self.__training, downsample=True)

            # residual block part4
            conv = layer.residual_block(name='residual19', input_data=conv, input_channel=1024,
                                           filter_num1=512, filter_num2=1024, training=self.__training)
            conv = layer.residual_block(name='residual20', input_data=conv, input_channel=1024,
                                           filter_num1=512, filter_num2=1024, training=self.__training)
            conv = layer.residual_block(name='residual21', input_data=conv, input_channel=1024,
                                           filter_num1=512, filter_num2=1024, training=self.__training)
            conv = layer.residual_block(name='residual22', input_data=conv, input_channel=1024,
                                           filter_num1=512, filter_num2=1024, training=self.__training)

        # conv经过几个卷积层之后作为检测分支的输入，这几个卷积层不改变输入conv的的shape
        conv = layer.convolutional(name='conv52', input_data=conv, filters_shape=(1, 1, 1024, 512),
                                      training=self.__training)
        conv = layer.convolutional(name='conv53', input_data=conv, filters_shape=(3, 3, 512, 1024),
                                      training=self.__training)
        conv = layer.convolutional(name='conv54', input_data=conv, filters_shape=(1, 1, 1024, 512),
                                      training=self.__training)
        conv = layer.convolutional(name='conv55', input_data=conv, filters_shape=(3, 3, 512, 1024),
                                      training=self.__training)
        conv = layer.convolutional(name='conv56', input_data=conv, filters_shape=(1, 1, 1024, 512),
                                      training=self.__training)

        # ----------**********---------- Detection branch of large object ----------**********----------
        conv_lobj_branch = layer.convolutional(name='conv_lobj_branch', input_data=conv,
                                                  filters_shape=(3, 3, 512, 1024), training=self.__training)
        conv_lbbox = layer.convolutional(name='conv_lbbox', input_data=conv_lobj_branch,
        ...
        pred_lbbox = layer.decode(name='pred_lbbox', conv_output=conv_lbbox, anchors=self.__anchors[2],
        ...
        conv = layer.convolutional(name='conv57', input_data=conv, filters_shape=(1, 1, 512, 256),
                                      training=self.__training)
        conv = layer.upsample(name='upsample0', input_data=conv)
        conv = layer.route(name='route0', previous_output=darknet_route2, current_output=conv)
        # ----------**********---------- up sample and merge features map ----------**********----------

        # up sample后的conv经过几个卷积层之后作为检测分支的输入，这几个卷积层不改变输入conv的的shape
        conv = layer.convolutional('conv58', input_data=conv, filters_shape=(1, 1, 512+256, 256),
                                      training=self.__training)
        conv = layer.convolutional('conv59', input_data=conv, filters_shape=(3, 3, 256, 512),
                                      training=self.__training)
        conv = layer.convolutional('conv60', input_data=conv, filters_shape=(1, 1, 512, 256),
                                      training=self.__training)
        conv = layer.convolutional('conv61', input_data=conv, filters_shape=(3, 3, 256, 512),
                                      training=self.__training)
        conv = layer.convolutional('conv62', input_data=conv, filters_shape=(1, 1, 512, 256),
                                      training=self.__training)


        # ----------**********---------- Detection branch of middle object ----------**********----------
        conv_mobj_branch = layer.convolutional(name='conv_mobj_branch', input_data=conv,
                                                  filters_shape=(3, 3, 256, 512), training=self.__training)
        conv_mbbox = layer.convolutional(name='conv_mbbox', input_data=conv_mobj_branch,
                                            filters_shape=(1, 1, 512, self.__anchor_per_scale * (self.__num_classes + 5)),
                                            training=self.__training, downsample=False, activate=False, bn=False)
        pred_mbbox = layer.decode(name='pred_mbbox', conv_output=conv_mbbox, anchors=self.__anchors[1],
                                     num_classes=self.__num_classes, stride=self.__strides[1])
        # ----------**********---------- Detection branch of middle object ----------**********----------

        # ----------**********---------- up sample and merge features map ----------**********----------
        # up sample之前用1x1的卷积将conv的channel变为128，以与darknet_route2的channel匹配
        conv = layer.convolutional(name='conv63', input_data=conv, filters_shape=(1, 1, 256, 128),
                                      training=self.__training)
        conv = layer.upsample(name='upsample1', input_data=conv)
        conv = layer.route(name='route1', previous_output=darknet_route1, current_output=conv)
        # ----------**********---------- up sample and merge features map ----------**********----------

        # up sample后的conv经过几个卷积层之后作为检测分支的输入，这几个卷积层不改变输入conv的的shape
        conv = layer.convolutional(name='conv64', input_data=conv, filters_shape=(1, 1, 256+128, 128),
                                      training=self.__training)
        conv = layer.convolutional(name='conv65', input_data=conv, filters_shape=(3, 3, 128, 256),
                                      training=self.__training)
        conv = layer.convolutional(name='conv66', input_data=conv, filters_shape=(1, 1, 256, 128),
                                      training=self.__training)
        conv = layer.convolutional(name='conv67', input_data=conv, filters_shape=(3, 3, 128, 256),
                                      training=self.__training)
        conv = layer.convolutional(name='conv68', input_data=conv, filters_shape=(1, 1, 256, 128),
                                      training=self.__training)

        # ----------**********---------- Detection branch of small object ----------**********----------
        conv_sobj_branch = layer.convolutional(name='conv_sobj_branch', input_data=conv,
                                                  filters_shape=(3, 3, 128, 256), training=self.__training)
        conv_sbbox = layer.convolutional(name='conv_sbbox', input_data=conv_sobj_branch,
                                            filters_shape=(1, 1, 256, self.__anchor_per_scale * (self.__num_classes + 5)),
                                            training=self.__training, downsample=False, activate=False, bn=False)
        pred_sbbox = layer.decode(name='pred_sbbox', conv_output=conv_sbbox, anchors=self.__anchors[0],
