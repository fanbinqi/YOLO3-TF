...
class YoloTest(object):
    def __init__(self, test_weights):
        ...
        self.__test_weight_path = os.path.join(cfg.WEIGHTS_DIR, test_weights)
        ...
        _, _, _, self.__pred_sbbox, self.__pred_mbbox, self.__pred_lbbox = \
            YOLOV3(self.__training).build_nework(self.__input_data)
        ...
        self.__saver = tf.train.Saver(net_vars)
        self.__saver.restore(self.__sess, self.__test_weight_path)
    ...
    def __get_bbox(self, image):
        ...
        if self.__multi_test:
            test_input_sizes = self.__train_input_sizes#self.__train_input_sizes[::3]
            bboxes_list = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--map_calc', default=True, type=str2bool)
    parser.add_argument('--test_2012', default=False, type=str2bool)
    parser.add_argument('--weights_file', default='yolo.ckpt-21-31.5124', type=str)
    ...
    if map_calc:
        ...
        cv2.imshow('detect_result.jpg', image)
        cv2.waitKey(0)
